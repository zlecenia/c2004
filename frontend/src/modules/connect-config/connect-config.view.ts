...
export class ConnectConfigView {
  ...
  render(): HTMLElement {
    ...
    container.innerHTML = `
      <div class="module-with-menu">
        <div id="connect-config-menu-container">
    ...
    if (menuContainer) {
      createModuleMenu('connect-config', menuContainer, {
        onItemClick: (data) => {
          ...
          console.log(`🔧 ConnectConfig Menu: ${item.action} - ${item.id} in column ${column.id}`);
          
          if (item.section) {
            this.currentCategory = item.section;
            this.updateCategoryContent();
          }
          
          if (item.subsection) {
            this.currentSubSection = item.subsection;
            this.updateSubsectionContent();
          }
        }
      });
    }
  ...
  private updateCategoryContent(): void {
    console.log(`🔧 ConnectConfig: Updating category content - ${this.currentCategory}`);
    
    // Update main content based on category selection
    const contentContainer = document.getElementById('connect-config-content');
    if (contentContainer) {
      const mainContent = contentContainer.querySelector('.content-body');
      if (mainContent) {
        // Show/hide relevant components based on category
        switch (this.currentCategory) {
          case 'system':
            mainContent.innerHTML = this.systemComponent.renderContent();
            break;
          case 'devices':
            mainContent.innerHTML = this.devicesComponent.renderContent();
            break;
          case 'security':
            mainContent.innerHTML = this.securityComponent.renderContent();
            break;
        }
      }
    }
  ...
  private updateSubsectionContent(): void {
    console.log(`🔧 ConnectConfig: Updating subsection content - ${this.currentCategory}/${this.currentSubSection}`);
    
    // Update content based on specific subsection
    const contentContainer = document.getElementById('connect-config-content');
    if (contentContainer) {
      // Add breadcrumb navigation
      const breadcrumb = `
        <div class="menu-breadcrumb">
          <span class="breadcrumb-item">${this.getCategoryLabel(this.currentCategory)}</span>
          <span class="breadcrumb-separator">></span>
          <span class="breadcrumb-item breadcrumb-active">${this.getSubsectionLabel(this.currentSubSection)}</span>
        </div>
      `;
      
      const existingBreadcrumb = contentContainer.querySelector('.menu-breadcrumb');
      if (existingBreadcrumb) {
        existingBreadcrumb.remove();
      }
      
      contentContainer.insertAdjacentHTML('afterbegin', breadcrumb);
    }
  }
  ...
  private getSubsectionLabel(subsection: string): string {
    const labels: Record<string, string> = {
      'performance': 'Wydajność',
      'network': 'Sieć',
      'updates': 'Aktualizacje'
