...
export class ConnectWorkshopView {
  ...
  private switchSection(section: string, container: HTMLElement): void {
    ...
    // Update title
    const titles = {
      'requests': 'Workshop - Requests',
      'transport': 'Workshop - Transport',
      'dispositions': 'Workshop - Dispositions',
      'service': 'Workshop - Service'
    };
    const titleEl = container.querySelector('#content-title');
    if (titleEl) {
      titleEl.textContent = titles[section as keyof typeof titles] || 'Workshop';
    }
  }
