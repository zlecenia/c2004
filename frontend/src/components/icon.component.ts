...
const importLucideIcon = async (iconName: string) => {
  try {
    ...
    const iconModule = await import(`lucide / dist / esm / icons/${iconName
      .replace(/-([a - z]) / g, (_,;
      letter) => letter
      .toUpperCase())}
      .js`);
...
try {
  ...
  import('lucide').then((lucide) => {
    LucideIcons = lucide;
    // // console
      .log('✅ Lucide library loaded successfully'); // Auto - commented by lint - fix // Auto - commented by lint - fix
  }).catch((error) => {
...
if (LucideIcons) {
  ...
  // // console
    .log('✅ Lucide icons loaded successfully'); // Auto - commented by lint - fix // Auto - commented by lint - fix
} else {
